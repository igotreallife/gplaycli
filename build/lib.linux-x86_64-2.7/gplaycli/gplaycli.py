...
class GPlaycli(object):
    ...
    def download_selection(self, playstore_api, list_of_packages_to_download, return_function):
        print list_of_packages_to_download
        ...
        print [item for item, item2 in list_of_packages_to_download]
    ...
    def raw_search(self, results_list, search_string, nb_results):
        # Query results
        #print self.playstore_api.search(search_string, nb_results=nb_results).doc
    ...
    def search(self, results_list, search_string, nb_results, free_only=True, include_headers=True):
        ...
        for docs in results:
            for result in docs.child:
                ...
                #printlist.append(str(result.docid))
                if len(all_results) < int(nb_results)+1:
                    all_results.append(l)
                    # beispiel: aus prepare_download_updates: list_of_packages_to_download.append([packagename, filename])


        #print printlist

        # beispiel: aus prepare_download_updates: list_of_packages_to_download.append([packagename, filename])
        #self.download_packages(printlist)
    ...
    #def category(self, nb_results, category, download):
    def category(self):

        result = self.playstore_api.browse()
        #print result
        display_categoy = []
        helper  = list()
        list_of_packages_to_download = []
        ...
        for c in result.category:
            c.name = c.name.replace(" & ", "_AND_")
            c.name = c.name.replace(" ", "_")
            c.name = c.name.upper()
            #j.write(str(c.name) + ":" + "\n")
            helper = self.playstore_api.list(str(c.name).upper())
            ...
            if str(c.name) != "GAMES":
                for x in range(0,2):
                    ...
                    try:
                        helper2 = self.playstore_api.list(str(c.name).upper(),helper.doc[x].docid, "100")
                        # to test detail
                        #list_to_call.append(str(helper2.doc[x].child[0].docid))
                    except IndexError:
                        break

                    t.write("\n" + str(c.name)+ "\n" + str(helper2))
                    t.write("###################               " + str(c.name) + str(x) + "                  ###################")

        t.close()
        #to test detail
        #self.detail(list_to_call)
        return display_categoy


    def detail(self, list_of_apks):
        details = open("details.txt", "w")
        list_of_apks = str(list_of_apks)
        list_of_apks= list_of_apks.strip('[')
        list_of_apks= list_of_apks.strip(']')
        list_of_apks= list_of_apks.strip(' ')
        lst = list_of_apks.split(',')
        for x in lst:
            print x
...
def main():
    ...
    parser.add_argument('-ca', '--categories', action='store_true', dest='categories',
                        help='Browse the fresh section')
    parser.add_argument('-de', '--detail', action='store_true', dest='detail',
                    help='Browse the fresh section')
    #parser.add_argument('-ta', '--detail', action='store', dest='gimme_list', metavar="AppID", nargs="+",
    #                    type=str, help='Get details of list of apps')
    ...
    if args.search_string:
        ...
        #list_results = cli.search(list(), args.search_string, nb_results, not args.paid)
        #cli.set_download_folder(args.dest_folder[0])
        #print list_results[1][5]

        # instant download
        # To strip the table of unused content s
        #list_to_download =[]
        #for x in range(1,(len(list_results)-1)):
        #    #print list_results[x][5]
        #    list_to_download.append(str(list_results[x][5]))
        #cli.download_packages(list_to_download)
